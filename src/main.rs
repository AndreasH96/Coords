...
fn main() {
    ...
    for i in (0..(text_hex.len())).step_by(window_size) {
        let window_size = std::cmp::min(window_size, text_hex.len() - i); // Adjust size for last window
        let window = text_hex
            .chars()
            .skip(i)
            .take(window_size)
        ...
        coordinates.push(generate_coordinate_for_distance(
            start,i32::from_str_radix(&window, 16).unwrap()
            ,
        ))
    }
    ...
    hex::decode(
            coordinates
                .iter()
                .map(|x| format!("{:x}", start.distance(x).floor() as i32))
                .join(""),
        )
        .unwrap(),
    )
