...
pub(crate) mod lib_tests {
    ...
    use coords::{encode,decode,generate_coordinate_for_distance};
    use nav_types::WGS84;
    use hex;
    ...
    fn test_encoding_decoding_long_text() {
        ...
        let original_text = fs::read_to_string("./tests/files/test_text.txt").expect("Should have been able to read the file");
    ...
    fn test_caching() {
        ...
        let coord1 = generate_coordinate_for_distance(origin, test_value);
        let coord2 = generate_coordinate_for_distance(origin, test_value);

        assert_eq!(coord1, coord2, "Cache did not return the same coordinate");
    }
}